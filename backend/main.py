...
async def scan_repo(req: RepoScanRequest):
    ...
    start = time.time()
    try:
        files = fetch_repo_files(repo_url=req.repo_url, branch=req.branch or "main",
            max_files=min(req.max_files or 20, settings.MAX_FILES_PER_SCAN), extensions=req.extensions)
    ...
    return ScanResponse(repo_url=req.repo_url, files_scanned=len(results), total_files=len(files),
        results=results, all_vulnerabilities=[Vulnerability(**v) for v in all_vulns_flat],
        summary=_build_summary(all_vulns_flat), scan_duration_seconds=round(time.time()-start, 2))
...
async def scan_file(req: FileScanRequest):
    ...
    return ScanResponse(filename=req.filename, files_scanned=1, total_files=1,
        results=[result], all_vulnerabilities=result.all_vulns,
        summary=_build_summary(all_vulns_flat), scan_duration_seconds=round(time.time()-start, 2))

@app.post("/api/scan/upload", response_model=ScanResponse)
async def scan_upload(files: List[UploadFile] = File(...)):
    if not settings.GROQ_API_KEY: raise HTTPException(500, "GROQ_API_KEY not configured.")
    EXT_LANG = {"py":"python","js":"javascript","ts":"typescript","java":"java",
    ...
    return ScanResponse(files_scanned=len(all_results), total_files=len(files),
        results=all_results, all_vulnerabilities=[Vulnerability(**v) for v in all_vulns],
...
async def ask_ai(req: AskRequest):
    if not settings.GROQ_API_KEY: raise HTTPException(500, "GROQ_API_KEY not configured.")
    try:
        answer = ask_about_vuln(question=req.question, vuln_type=req.vuln_type,
            cwe=req.cwe, file_path=req.file_path, description=req.description)
